...
import { CloseIcon, SkillsIcon } from '../icons'
import type { CreateSkillDto, Skill } from '../../types/skill'
...
export function AddSkillModal({ isOpen, onClose, onSkillCreated }: AddSkillModalProps) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (response.success && response.payload) {
        ...
        onSkillCreated(response.payload)
        handleClose()
        return
      }
      toast.error(response.error?.message || 'Failed to create skill')
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'An error occurred while creating skill')
    }
  }
      ...
      <div className="relative z-10 w-full max-w-lg transform overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl transition-all animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900">Add Technician Skill</h3>
              <p className="text-xs text-slate-500">Define a technical skill, certification, or specialization</p>
            </div>
          </div>
          <button
            type="button"
